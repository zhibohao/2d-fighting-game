...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	if (iIndex < m_ppDisplayableObjects.size()) {
		m_ppDisplayableObjects.at(iIndex) = pObject;
	}
}

void BaseEngine::AddObject(DisplayableObject* pObject)
{
	if (m_ppDisplayableObjects.empty()) {
		m_ppDisplayableObjects.reserve(1);
	}
...
void BaseEngine::DestroyOldObjects()
{
	...
	if (!m_ppDisplayableObjects.empty())
	{
		...
		{
			if (m_ppDisplayableObjects.at(i) != nullptr)
			{
				delete m_ppDisplayableObjects.at(i);
				m_ppDisplayableObjects.at(i) = NULL;
			}
		}
...
void BaseEngine::UpdateAllObjects(int iCurrentTime)
{
	...
	for (int i = 0; i < m_ppDisplayableObjects.size(); i++)
	{
		if (m_ppDisplayableObjects.at(i) == nullptr) continue;
